...
impl Display for Live {
    fn show(width: usize, height: usize, fractal_generator: Box<dyn FractalGenerator>) {
        let mut buffer: Vec<u32> = Live::get_fractal_buffer(&fractal_generator, width, height);
        ...
        while window.is_open() && !window.is_key_down(Key::Escape) {
            ...
            if update_is_needed {
                buffer = Live::get_fractal_buffer(&fractal_generator, width, height);
...
impl Live {
    ...
    fn get_fractal_buffer(fractal_generator: &Box<dyn FractalGenerator>, width: usize, height: usize) -> Vec<u32> {
